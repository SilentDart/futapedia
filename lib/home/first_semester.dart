...
class FirebaseQueryCacheFirstSemester {
...
class _FirstSemesterState extends State<FirstSemester> with AutomaticKeepAliveClientMixin {
  String? scholar;
  String? userImageUrl;
  ...
  final int _cacheDurationDays = 7; // 30-day cache expiration
  ...
  void _loadSemesterAndCourses({bool forceRefresh = false}) async {
    ...
    if (!forceRefresh && await FirebaseQueryCacheFirstSemester.isCacheValid(_semesterCacheKey, durationDays: _cacheDurationDays)) {
      final cachedSemester = await FirebaseQueryCacheFirstSemester.getCachedString(_semesterCacheKey);
    ...
    try {
      ...
      if (result != null && result.isNotEmpty) {
        ...
        // Cache the semester
        await FirebaseQueryCacheFirstSemester.saveQueryResult(_semesterCacheKey, result);
  ...
  Future<void> _loadCoursesWithCache({bool forceRefresh = false}) async {
    ...
    if (!forceRefresh) {
      if (await FirebaseQueryCacheFirstSemester.isCacheValid(_level100CacheKey, durationDays: _cacheDurationDays)) {
        final cachedCourses = await FirebaseQueryCacheFirstSemester.getCachedStringList(_level100CacheKey);
      ...
      if (await FirebaseQueryCacheFirstSemester.isCacheValid(_level300CacheKey, durationDays: _cacheDurationDays)) {
        final cachedCourses = await FirebaseQueryCacheFirstSemester.getCachedStringList(_level300CacheKey);
        if (cachedCourses != null && cachedCourses.isNotEmpty) {
          setState(() {
      ...
      if (await FirebaseQueryCacheFirstSemester.isCacheValid(_level200CacheKey, durationDays: _cacheDurationDays)) {
        final cachedCourses = await FirebaseQueryCacheFirstSemester.getCachedStringList(_level200CacheKey);
        if (cachedCourses != null && cachedCourses.isNotEmpty) {
          setState(() {
    ...
    try {
      ...
      if (needToFetchLevel100) {
        ...
        // Cache the results
        await FirebaseQueryCacheFirstSemester.saveQueryResult(_level100CacheKey, courses100);
      ...
      if (needToFetchLevel200) {
        ...
        // Cache the results
        await FirebaseQueryCacheFirstSemester.saveQueryResult(_level200CacheKey, courses200);
      ...
      if (needToFetchLevel300) {
        ...
        // Cache the results
        await FirebaseQueryCacheFirstSemester.saveQueryResult(_level300CacheKey, courses300);
  ...
  Widget _buildHorizontalCourseList() {
    ...
    if (level100Courses.isEmpty && level200Courses.isEmpty) {
      return SizedBox(
        height: 90,
        child: Center(
          child: Text(
